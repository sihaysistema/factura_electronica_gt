...
frappe.query_reports["GT Sales Ledger"] = {
    "filters": [{
        ...
        "reqd": 1,
        on_change: function () {
            frappe.db.get_value('Company', frappe.query_report.get_filter_value('company'),
                'tax_id')
                .then(r => {
                    // console.log(r.message.name)
                    frappe.query_report.set_filter_value('nit', r.message.tax_id);
                    frappe.db.get_value('Company', frappe.query_report.get_filter_value(
                        'company'), 'default_currency')
                        .then(r => {
                            frappe.query_report.set_filter_value('company_currency', r
