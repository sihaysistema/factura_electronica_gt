            ...
            if (item_row.factelecis_fuel === 1) {
                ...
                $.each(frm.doc.items || [], function (i, d) {
                    if (d.factelecis_fuel == true) {
                        _total_fuel += flt(d.facelec_gt_tax_net_fuel_amt);
                    };
                });

                frm.doc.facelec_gt_tax_fuel = _total_fuel;
            ...
            if (item_row.facelec_is_good === 1) {
                ...
                $.each(frm.doc.items || [], function (i, d) {
                    if (d.facelec_is_good == true) {
                        _total_goods += flt(d.facelec_gt_tax_net_goods_amt);
                    };
                });

                frm.doc.facelec_gt_tax_goods = _total_goods;
            };
            if (item_row.facelec_is_service === 1) {
                ...
                $.each(frm.doc.items || [], function (i, d) {
                    if (d.facelec_is_service == true) {
                        _total_servi += flt(d.facelec_gt_tax_net_services_amt);
                    };
                });

                frm.doc.facelec_gt_tax_services = _total_servi;
            };

            var _full_tax_iva = 0;
            $.each(frm.doc.items || [], function (i, d) {
                _full_tax_iva += flt(d.facelec_sales_tax_for_this_row);
            });
            frm.doc.facelec_total_iva = _full_tax_iva;
        };
    });
}
...
function pdf_button(cae_documento, frm) {
    frm.add_custom_button(__("Actualizar Datos Factura"), function () {
        window.open("https://www.ingface.net/Ingfacereport/dtefactura.jsp?cae=" + cae_documento);
    }).addClass("btn-primary");
}
            ...
            if (item_row.shs_so_is_fuel === 1) {
                ...
                $.each(frm.doc.items || [], function (i, d) {
                    if (d.shs_so_is_fuel === 1) {
                        _total_fuel2 += flt(d.shs_so_gt_tax_net_fuel_amt);
                    };
                });
                frm.doc.shs_gt_tax_fuel = _total_fuel2;
            };
            if (item_row.shs_so_is_good === 1) {
                ...
                $.each(frm.doc.items || [], function (i, d) {
                    if (d.shs_so_is_good === 1) {
                        _total_goods2 += flt(d.shs_so_gt_tax_net_goods_amt);
                    };
                });
                frm.doc.shs_so_gt_tax_goods = _total_goods2;
            };
            if (item_row.shs_so_is_service === 1) {
                ...
                $.each(frm.doc.items || [], function (i, d) {
                    if (d.shs_so_is_service === 1) {
                        _total_servi2 += flt(d.shs_so_gt_tax_net_services_amt);
                    };
                });
                frm.doc.shs_so_gt_tax_services = _total_servi2;
            };
            var _full_tax_iva2 = 0;
            $.each(frm.doc.items || [], function (i, d) {
                _full_tax_iva2 += flt(d.shs_so_sales_tax_for_this_row);
            });
            frm.doc.shs_so_total_iva = _full_tax_iva2;
        };
    });
}
