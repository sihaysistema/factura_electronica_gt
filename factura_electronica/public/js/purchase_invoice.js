        ...
        if (item_row_i.name === cdn) {
            ...
            if (cuenta) {
                ...
                if (!pi_buscar_cuenta(frm, cuenta)) {
                    // Si no encuentra una cuenta, procede.
                    ...
                    // Si la cuenta ya esta agregada en shs_pi_otros_impuestos, se procede a sumar sobre los valores
                    // ya existentes
                    // Funcion que se encarda de sumar los valores por cuenta
                    pi_sumatoria_otros_impuestos_por_cuenta(frm, cdt, cdn);
                    cur_frm.refresh_field("shs_pi_otros_impuestos");
...
function shs_purchase_invoice_calculation(frm, cdt, cdn) {
    ...
    frm.doc.items.forEach((item_row, index) => {
        if (item_row.name == cdn) {
            ...
            frm.doc.items[index].facelec_p_other_tax_amount =
                item_row.facelec_p_tax_rate_per_uom * (item_row.qty * item_row.conversion_factor);
            //OJO!  No s epuede utilizar stock_qty en los calculos, debe de ser qty a puro tubo!
            frm.doc.items[index].facelec_p_amount_minus_excise_tax =
                item_row.qty * item_row.rate - item_row.qty * item_row.conversion_factor * item_row.facelec_p_tax_rate_per_uom;

            if (item_row.facelec_p_is_fuel) {
                frm.doc.items[index].facelec_p_gt_tax_net_fuel_amt =
                    item_row.facelec_p_amount_minus_excise_tax / (1 + this_company_sales_tax_var / 100);
                frm.doc.items[index].facelec_p_sales_tax_for_this_row =
                    item_row.facelec_p_gt_tax_net_fuel_amt * (this_company_sales_tax_var / 100);
            ...
            if (item_row.facelec_p_is_good) {
                frm.doc.items[index].facelec_p_gt_tax_net_goods_amt =
                    item_row.facelec_p_amount_minus_excise_tax / (1 + this_company_sales_tax_var / 100);
                frm.doc.items[index].facelec_p_sales_tax_for_this_row =
                    item_row.facelec_p_gt_tax_net_goods_amt * (this_company_sales_tax_var / 100);
            ...
            if (item_row.facelec_p_is_service == 1) {
                frm.doc.items[index].facelec_p_gt_tax_net_services_amt =
                    item_row.facelec_p_amount_minus_excise_tax / (1 + this_company_sales_tax_var / 100);
                frm.doc.items[index].facelec_p_sales_tax_for_this_row =
                    item_row.facelec_p_gt_tax_net_services_amt * (this_company_sales_tax_var / 100);
...
frappe.ui.form.on("Purchase Invoice", {
    refresh: function (frm, cdt, cdn) {
        ...
        // INICIO BOTON PARA GENERAR NOTA DE DEBITO ELECTRONICA
        ...
        // FIN BOTON PARA GENERAR NOTA DE DEBITO ELECTRONICA
    },
    refresh: function (frm, cdt, cdn) {
        ...
        if (frm.doc.docstatus === 1) {
            ...
            frm.add_custom_button(__("Generate Special Invoice FEL"), function () {
                frappe.confirm(
                    __("Are you sure you want to proceed to generate a Special Invoice?"),
                    () => {
                        frappe.call({
...
frappe.ui.form.on("Purchase Invoice Item", {
    before_items_remove: function (frm, cdt, cdn) {
        frm.doc.items.forEach((item_row_1, index_1) => {
            if (item_row_1.name == cdn) {
                pi_total_otros_impuestos_eliminacion(
                    frm,
                    item_row_1.facelec_p_tax_rate_per_uom_account,
